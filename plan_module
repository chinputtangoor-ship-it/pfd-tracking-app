...
def show_plan_page(load_csv, save_to_csv, update_full_sheet):
    ...
    # เช็ค session state สำหรับโหมดแก้ไข
    if 'editing_batch' not in st.session_state:
        st.session_state.editing_batch = None

    # ==========================================
    # ส่วนที่ 1: หน้าจอแก้ไข (จะแสดงทับเมื่อกดปุ่มแก้ไขเท่านั้น)
    # ==========================================
    if st.session_state.editing_batch is not None:
        idx = st.session_state.editing_batch
        row = plan_df.iloc[idx]
        st.subheader(f"📝 แก้ไขข้อมูล Batch: {row['batch_number']}")

        with st.form("edit_mode_form"):
            # กาง 23 คอลัมน์สำหรับแก้ไข
            st.markdown("##### 📦 ข้อมูลการผลิต")
            r1c1, r1c2, r1c3, r1c4 = st.columns(4)
            e_line = r1c1.selectbox("Line", LINES, index=LINES.index(row['line']) if row['line'] in LINES else 0)
            e_batch = r1c2.text_input("Batch Number", value=str(row['batch_number']))
            e_sap = r1c3.text_input("SAP Batch", value=str(row['sap_batch']))
            e_order = r1c4.text_input("Prod. Order", value=str(row['production_order']))

            r2c1, r2c2, r2c3, r2c4 = st.columns(4)
            e_inspec = r2c1.text_input("Inspec. Lot", value=str(row['inspection_lot']))
            e_so = r2c2.text_input("Sales Order", value=str(row['sales_order']))
            e_so_item = r2c3.text_input("SO Item", value=str(row['sales_order_item']))
            e_fert = r2c4.text_input("FERT Code", value=str(row['fert_code']))

            r3c1, r3c2, r3c3, r3c4 = st.columns(4)
            e_semi = r3c1.text_input("Semi Code", value=str(row['semifinish_code']))
            e_qty = r3c2.number_input("Item Qty (K)", value=float(row['item_qty']), format="%.3f")
            e_af = r3c3.number_input("Need AF Box", value=float(row['need_af_box']), format="%.0f")
            e_cust = r3c4.selectbox("Customer", CUSTOMER_NAMES, index=CUSTOMER_NAMES.index(row['customer_name']) if row[
                                                                                                                        'customer_name'] in CUSTOMER_NAMES else 0)

            st.markdown("##### 🎨 รายละเอียดบรรจุภัณฑ์")
            r4c1, r4c2, r4c3, r4c4 = st.columns(4)
            e_finish = r4c1.date_input("Plan Finish", value=pd.to_datetime(row['planned_finish_date']))
            e_desp = r4c2.date_input("To be Desp.", value=pd.to_datetime(row['to_be_desp_on']))
            e_metal = r4c3.selectbox("Metal Det.", METAL_DETECTOR_OPTIONS,
                                     index=METAL_DETECTOR_OPTIONS.index(row['metal_detector']) if row[
                                                                                                      'metal_detector'] in METAL_DETECTOR_OPTIONS else 0)
            e_print = r4c4.selectbox("Print Type", ["U", "P", "N/A"],
                                     index=["U", "P", "N/A"].index(row['print_type']) if row['print_type'] in ["U", "P",
                                                                                                               "N/A"] else 0)
            ...
            e_box = r5c2.selectbox("Box Packing", BOX_PACKING_OPTIONS,
                                   index=BOX_PACKING_OPTIONS.index(row['box_packing']) if row[
                                                                                              'box_packing'] in BOX_PACKING_OPTIONS else 0)
            e_ink_cap = r5c3.text_input("Ink Cap", value=str(row['ink_cap']))
            e_rol_cap = r5c4.text_input("Roller Des Cap", value=str(row['roller_des_cap']))

            r6c1, r6c2, r6c3, r6c4 = st.columns(4)
            e_ink_body = r6c1.text_input("Ink Body", value=str(row['ink_body']))
            e_rol_body = r6c2.text_input("Roller Des Body", value=str(row['roller_des_body']))
            e_status = r6c3.selectbox("Status", ["Running", "Finished"],
                                      index=0 if row['batch_status'] == "Running" else 1)
    ...
    # ==========================================
    # ส่วนที่ 2: หน้าหลัก 3 Tabs
    # ==========================================
    t_view, t_add, t_finish = st.tabs(["🔍 ดูแผนงาน", "➕ เพิ่มแผนการผลิตใหม่", "🔄 จบงาน"])
    ...
    with t_view:
        ...
        if not show_finished:
            display_df = display_df[display_df['batch_status'] == "Running"]
            ...
            if st.form_submit_button("➕ บันทึกแผนงานใหม่", use_container_width=False, type="primary"):
                if n_batch:
                    new_row = [n_line, n_batch, n_sap, n_order, n_inspec, n_so, n_so_item, n_fert, n_semi, n_qty, n_af,
                               ...
                               n_rol_cap, n_ink_body, n_rol_body, "Running"]
